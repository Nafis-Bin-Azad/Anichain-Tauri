...
lazy_static! {
    ...
    static ref EPISODE_PATTERNS: Vec<(&'static str, Regex)> = vec![
        ...
        ("AniDB", Regex::new(r"(?ix)(?:\[(?P<group>[^\]]+)\])?[^\[]*?(?P<title>[^-\[]+?)(?:\s*[-\[]|\s+(?:EP|Episode|第)\s*)?(?P<episode>\d{1,3})(?:v\d)?(?:\s*(?:\[(?:[^\]]*)\]|\([^\)]*\)))*").unwrap()),
        
        // TVDB style: Show.Title.S01E01.Episode.Title.mkv
        ("TVDB", Regex::new(r"(?ix)(?P<title>.+?)(?:\.|\s+)?(?:S(?P<season>\d{1,2})?(?:E|x)(?P<episode>\d{1,2}))").unwrap()),
        
        // Absolute style: Show Title - 01.mkv
        ("Absolute", Regex::new(r"(?ix)(?P<title>[^-]+?)\s*[-\s.]+\s*(?P<episode>\d{1,3})(?:v\d)?").unwrap()),
        
        // Special episodes
        ("Special", Regex::new(r"(?ix)(?:\[(?P<group>[^\]]+)\])?[^\[]*?(?P<title>[^-\[]+?)(?:\s*[-\[]|\s+)?(?P<special>S(?:pecial)?|OVA|OP|ED|NCOP|NCED|Preview|Movie)(?:\s*[-\[]|\s+)?(?P<episode>\d{1,3})?").unwrap()),
        
        // Movie pattern
        ("Movie", Regex::new(r"(?ix)(?:\[(?P<group>[^\]]+)\])?[^\[]*?(?P<title>[^-\[]+?)(?:\s*(?:Movie|劇場版|完全版))?(?:\s*[-\[]|\s+)?(?:\s*\((?P<year>\d{4})\))?").unwrap()),
    ];

    // Folder patterns
    static ref FOLDER_PATTERNS: Vec<(&'static str, Regex)> = vec![
        // AniDB style: [Group] Show Title (Year)
        ("AniDB", Regex::new(r"(?ix)(?:\[(?P<group>[^\]]+)\])?[^\[]*?(?P<title>[^-\[]+?)(?:\s*\((?P<year>\d{4})\))?(?:\s*\[(?P<source>BD|DVD|WEB)\])?").unwrap()),
        
        // Season style: Show Title Season 01
        ("Season", Regex::new(r"(?ix)(?P<title>.+?)(?:\s+(?:Season|S)\s*(?P<season>\d{1,2}))").unwrap()),
        
        // Year style: Show Title (2024)
        ("Year", Regex::new(r"(?ix)(?P<title>.+?)\s*\((?P<year>\d{4})\)").unwrap()),
        
        // Metadata ID style: Show Title [anidb-12345]
        ("MetadataID", Regex::new(r"(?ix)(?P<title>.+?)\s*\[(?:anidb|tvdb)-(?P<id>\d+)\]").unwrap()),
    ];

    // Quality pattern
    static ref QUALITY_PATTERN: Regex = Regex::new(r"(?i)(?:\[|\()(?P<quality>(?:\d{3,4}[pi]|SD|HD|FHD|UHD|4K|8K|BD|DVD|WEB)(?:-(?:Hi10P|10bit|HEVC|H\.?265|x265|AVC|H\.?264|x264))?)(?:\]|\))").unwrap();
}
...
pub async fn scan_anime_folder(root_path: &str) -> Result<Vec<AnimeFolder>> {
    ...
    // Create reqwest client for fetching images
...
async fn fetch_anime_image(client: &reqwest::Client, title: &str) -> Option<String> {
    ...
    let clean_title = clean_title.split(" - ").next()?.trim().to_string();
    let clean_title = clean_title.split("[").next()?.trim().to_string();

    tracing::info!("Fetching image for anime: {}", clean_title);
    ...
    tracing::debug!("Making request to Jikan API: {}", query_url);

    match client.get(&query_url).send().await {
        Ok(response) => {
            if !response.status().is_success() {
                tracing::error!(
                    "Failed to fetch anime image. Status: {}",
                    response.status()
                );
                return None;
            }
            ...
            match response.json::<serde_json::Value>().await {
                Ok(data) => {
                    ...
                    tracing::warn!("No image found in API response for: {}", clean_title);
                }
                Err(e) => {
        ...
        Err(e) => {
            tracing::error!("Failed to fetch anime image: {}", e);
        }
    }
...
fn scan_directory(dir: &Path, folders: &mut Vec<AnimeFolder>) -> Result<()> {
    ...
    if let Some(folder_name) = dir.file_name().map(|n| n.to_string_lossy()) {
                        ...
                        if let Ok(entry) = entry {
                            if let Some(file_name) = entry.file_name().to_str() {
                                if file_name.ends_with(".mkv") || file_name.ends_with(".mp4") {
                                    // Try to extract series name from the first video file
        ...
        if current_folder.title.is_empty() {
            for (pattern_name, pattern) in FOLDER_PATTERNS.iter() {
                if let Some(caps) = pattern.captures(&folder_name) {
                    tracing::debug!("Matched {} pattern", pattern_name);
                    ...
                    if let Some(season) = caps.name("season") {
                        ...
                        // Use year as season if no season specified
        ...
        if !found_match && current_folder.title.is_empty() {
            tracing::debug!("No folder pattern matched, using raw folder name");
                        ...
                        if let Some(ext) = path.extension() {
                            ...
                            if ["mp4", "mkv", "avi", "m4v"].contains(&ext.as_str()) {
                                if let Some(filename) = path.file_name().map(|n| n.to_string_lossy().to_string()) {
                                    tracing::debug!("Processing video file: '{}'", filename);
                                    ...
                                    for (pattern_name, pattern) in EPISODE_PATTERNS.iter() {
                                        if let Some(caps) = pattern.captures(&filename) {
                                            tracing::debug!("Matched {} pattern for file", pattern_name);
                                            
                                            // Extract quality using separate pattern
                                            ...
                                            // Update folder title if it's empty and we found a title in the episode
                                    ...
                                    if !episode_found {
                                        tracing::debug!("No pattern matched for file: '{}'", filename);
                                    }
                                }
                            }
                        }
